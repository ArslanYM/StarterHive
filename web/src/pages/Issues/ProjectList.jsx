...
const ProjectList = ({theme}) => {
  ...
  function filterProject(project) {
    ...
    const isTagsMatch = selectedLanguages.every((language) =>
      project.tags.includes(language)
    );

    // If the user wants to see only bookmark projects of a specific language
    if (showBookMark) {
      return isTagsMatch && bookMarkProjects.includes(project.projectLink);
    }
    ...
    <div className="mx-auto min-h-screen max-w-screen-2xl p-4 md:p-8 2xl:p-10 ">
      <div className="w-full my-4 p-4 flex items-center flex-wrap gap-4">
        ...
        <div
          ...
          className={!theme ? `${showBookMark ? 'border-yellow-400' : 'border-gray-600'}
           text-white flex items-center gap-2 border-2  px-2 py-1 rounded-md cursor-pointer hover:border-yellow-400 duration-300`: `${showBookMark ? 'border-yellow-400' : 'border-gray-600'}
           text-gray-600 flex items-center gap-2 border-2  px-2 py-1 rounded-md cursor-pointer hover:border-yellow-400 duration-300`}
        >
          {showBookMark ? (
          ...
          <span ref={observerRef} className="opacity-60 text-sm">
            Bookmark:{' '}
          </span>
      ...
      <div className={!theme ?"grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-4 xl:grid-cols-3 2xl:gap-7.5 bg-gradient-to-r from-gray-700 via-gray-900 to-black bg-animate place-items-center" :"grid grid-cols-1 gap-6 md:grid-cols-2 md:gap-4 xl:grid-cols-3 2xl:gap-7.5 bg-animate place-items-center"}>
        {bookMarkProjects &&
